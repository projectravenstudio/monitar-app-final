...
$query_registration_status = "SELECT status FROM settings WHERE id = 1";  // Assuming the status is stored in a settings table
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if username already exists in stud_tbl
    ...
    // Insert new student into stud_tbl (password stored as plaintext)
    $stmt = $conn->prepare("INSERT INTO stud_tbl (first_name, last_name, section, grade_level, username, password) 
                            VALUES (?, ?, ?, ?, ?, ?)");
